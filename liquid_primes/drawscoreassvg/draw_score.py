...
def _draw_score(
    ...
    for n, voice in enumerate(score.voices):
        ...
        if with_labels:
            ...
            d.append(
                draw.Text(text=voice.name, font_size=4, x=-min(config.score_view.margin, label_offset), y=label_y_pos)
            )
...
def _draw_line(positions, y_axis_visual_offset):
    ...
    if len(positions) == 2:
        ...
        return draw.Line(
            sx, sy + y_axis_visual_offset, ex, ey + y_axis_visual_offset, stroke="black", stroke_width=0.25, fill="none"
        )
    else:
