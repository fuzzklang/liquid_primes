from liquid_primes.score.create_score_with_gliss import map_to_gliss_events, get_sum_of_voices
...
def test_gliss_events_correctly_created():
    input_events = [
        Event(onset=1, pitch=60, duration=2),
        Event(onset=5, pitch=61, duration=2),
    ]
    result = map_to_gliss_events(
        events=input_events, distance_to_trigger_gliss=0, split_point_normalized=0.5, pitch_bend=1
    )
    expected = [
        GlissEvent(
            onset=1, start_pitch=60, gliss=[GlissPoint(61, duration=1.0), GlissPoint(end_pitch=60, duration=1.0)]
        ),
        GlissEvent(
            onset=5, start_pitch=61, gliss=[GlissPoint(62, duration=1.0), GlissPoint(end_pitch=61, duration=1.0)]
        ),
    ]
...
def test_gliss_events_trigger_distance_correct():
    input_events = [
        Event(onset=1, pitch=60, duration=2),
        Event(onset=2, pitch=60, duration=2),
        Event(onset=4, pitch=60, duration=2),
    ]
    result = map_to_gliss_events(
    ...
    expected = [
        ...
        GlissEvent(onset=2, start_pitch=60, gliss=[GlissPoint(end_pitch=60, duration=2.0)]),
        GlissEvent(
            onset=4, start_pitch=60, gliss=[GlissPoint(61, duration=1.0), GlissPoint(end_pitch=60, duration=1.0)]
        ),
    ]
    assert result == expected
...
def test_get_sum_of_voices():
    events_a = [GlissEvent(onset=1, start_pitch=1, gliss=[GlissPoint(1, 1)])]
    events_b = [GlissEvent(onset=3, start_pitch=1, gliss=[GlissPoint(1, 1)])]
    voices: list[Voice] = [
...
def test_map_to_drawable_score():
    voices = [
        Voice("a", "type_a", events=[GlissEvent(1, 60, [GlissPoint(60, 1)])]),
        Voice(
            "b",
            "type_b",
            events=[GlissEvent(1, 60, [GlissPoint(60, 1)]), GlissEvent(1, 60, [GlissPoint(61, 1), GlissPoint(60, 1)])],
        ),
    ]
